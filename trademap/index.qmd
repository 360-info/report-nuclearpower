...
sourceTotalOut = d3.sum(
  mergedData
    .filter(d => d.category == selectedCategory && d.source == selectedSource)
    .map(d => d.sum_value_usd))
sourceTotalIn = d3.sum(
  mergedData
    .filter(d => d.category == selectedCategory && d.recipient == selectedSource)
    .map(d => d.sum_value_usd))

recipientTotalOut = d3.sum(
  mergedData
    .filter(d => d.category == selectedCategory && d.source == selectedRecipient)
    .map(d => d.sum_value_usd))
recipientTotalIn = d3.sum(
  mergedData
    .filter(d => d.category == selectedCategory && d.recipient == selectedRecipient)
    .map(d => d.sum_value_usd))
